...
def get_data_for_town(town, strng)
  res = strng.split("\n")
       .select{ |line| line.include?("#{town}:") }
...
def mean(town, strng)
  data = get_data_for_town(town, strng)

  if !data.nil?
    sum = 0.0
    data.each { |val| sum += val.to_f }
    sum / 12.0
  else -1
  end
...
def variance(town, strng)
  ...
  if !data.nil?
    avg = mean(town, strng)
    sum_of_squares = 0.0
    data.each { |val| sum_of_squares += (val.to_f - avg) ** 2 }
    sum_of_squares / 12.0
  else -1
  end
...
p mean("London", data) #expected 51.199999999999996
p mean("Beijing", data) #expected 52.416666666666664
p mean("FDfsdfsdfhsdkjf", data) #expected -1
p variance("London", data) #expected 57.42833333333374
p variance("Beijing", data) #expectde 4808.37138888889
p variance("FDfsdfsdfhsdkjf", data) #expectde -1
