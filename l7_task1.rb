def mean(town, strng)
    ...
    av = 0.0
    tmp_ar.each {|x| av = av + x}
    av = av / 12.0 
    puts town + " :  average = "+ av.to_s[0..av.to_s.index(".")+2]
  end
  def variance(town, strng)
    ...
    av = av / 12
    dev = 0.0
    tmp_ar.each {|x| dev = dev + (x-av)**2}
    dev = Math.sqrt(dev) / 12.0 
    puts town + " :  deviation = "+ dev.to_s[0..dev.to_s.index(".")+2]
  end
