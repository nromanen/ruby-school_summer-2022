...
def splitStr(town, strng)

    split_str = strng.split("\n").select{ |element| element.include?("#{town}:") }

  if split_str.length != 0
    split_str[0].delete("#{town}:")
       .split(',')
       .map{ |duo| duo.split(' ') }
       .map{ |duo| duo[1] }
    else nil
...
def mean(town, strng)
    ...
    rain_value = splitStr(town, strng)

    sum = 0.0

    if !rain_value.nil?
        rain_value.each { |val| sum += val.to_f }
        sum / 12.0
      else -1
      end
  ...
  puts mean("end_mean: ", data)
  puts variance("London", data)
  puts variance("Sydney", data)
  puts variance("end_variance:", data)