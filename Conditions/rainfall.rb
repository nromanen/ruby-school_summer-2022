...
def mean(town, string)
  ...
  pattern = (/[0-9]+.[0-9]+/)
  year = 12
  # splitting text by condition
  hash = string.split("\n").map {|i| i.split(":")}.to_h
  !hash[town]==0 ? -1.0 : hash[town].scan(pattern).map {|i| i.to_f}.sum / year
end
...
def variance(town, string)
  ...
  # splitting text by condition
  hash = string.split("\n").map {|i| i.split(":")}.to_h
  !hash[town]==0 ? -1.0 : hash[town].scan(pattern).map {|i| i.to_f}.map {|i| (i - mean(town, string))**2 }.sum / year
end

# test1
mean = mean('Bangkok', data)
var = variance('Bangkok', data)
puts "Mean: #{mean}, Variance: #{var}\n\n"

# test2
mean = mean('Lima', data1)
var = variance('Lima', data1)
puts "Mean: #{mean}, Variance: #{var}\n\n"